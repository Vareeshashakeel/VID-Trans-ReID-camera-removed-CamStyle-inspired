...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='VID-Trans-ReID camera-removed + CamStyle-inspired augmentation training')
    ...
    parser.add_argument('--output_dir', default='./output_camera_removed_camstyle', help='Checkpoint output directory', type=str)
    parser.add_argument('--batch_size', default=64, help='Training batch size', type=int)
    parser.add_argument('--num_workers', default=4, help='Number of dataloader workers', type=int)
    parser.add_argument('--seq_len', default=4, help='Frames per sampled clip', type=int)
    parser.add_argument('--disable_camstyle', action='store_true', help='Disable CamStyle-inspired augmentation and fall back to the plain no-camera baseline')
    parser.add_argument('--camstyle_prob', default=0.5, help='Probability of applying shared camera-style augmentation to a training tracklet', type=float)
    parser.add_argument('--camstyle_strength', default=0.18, help='Strength of the camera-style augmentation', type=float)
    ...
    set_seed(1234)
    print(f'CamStyle-style augmentation: {not args.disable_camstyle} | prob={args.camstyle_prob:.2f} | strength={args.camstyle_strength:.2f}')


    train_loader, _, num_classes, camera_num, _, q_val_set, g_val_set = dataloader(
        args.Dataset_name,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        seq_len=args.seq_len,
        use_camstyle_aug=not args.disable_camstyle,
        camstyle_prob=args.camstyle_prob,
        camstyle_strength=args.camstyle_strength,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.eval_every == 0:
            ...
            latest_path = os.path.join(args.output_dir, f'{args.Dataset_name}_camera_removed_camstyle_latest.pth')
            torch.save(model.state_dict(), latest_path)
            if best_rank1 < rank1:
                best_rank1 = rank1
                best_path = os.path.join(args.output_dir, f'{args.Dataset_name}_camera_removed_camstyle_best.pth')
